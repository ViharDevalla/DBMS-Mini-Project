...
const signup = () => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        axios.post('/api/auth',{email:username,password:password})
        .then(res => {
            if(res.data.data){
    ...
    return (
        <div>
            <main className="flex flex-col items-center justify-center w-full flex-1 px-20 text-center lg:py-24">
                ...
                Signup to <a className="text-green-600">LibraSmart</a>
                </h1>
                <div className="w-96 p-8 border-2  rounded-lg">
                    <form className="flex flex-col " onSubmit={handleSubmit}>
                        <div className="flex flex-col text-left">
                            ...
                            <input className="w-full p-2 border" type="text" placeholder="Password" onChange={(e)=>{setPassword(e.target.value)}} />
                            ...
                            <input type="submit" className="bg-green-500 text-white px-2 py-2 rounded-lg text-center hover:bg-green-600 cursor-pointer" />
                        </div>
                    </form>
